...
ggplot()+
  ...
  geom_sf(data = roads_lines,
          ...
          alpha = .8)+
  labs(
    caption = "<span style='font-size: 30px;margin-bottom: 30px;'>City of</span><br><br><span style='font-size:85px;'>ISLAMABAD</span><br><br><span style='font-size:35px;'>33° 68' N - 73° 04' E</span><br><br>
       <span style='font-size:14px;'>Data : OpenStreetMap  **  Graphic: Azhar</span>"
  ) +
  theme_void()+
  theme(
    # CAPTION
    plot.caption = ggtext::element_markdown(hjust = .5,
                                            family = title_font,
                                            face = "bold"), 
    plot.background = element_rect(fill=bg_color, color="white", size=10),
